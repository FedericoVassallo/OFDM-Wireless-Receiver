...
% -------------------------------------------------------------------------
% Case: Comb Pilot (Recommended for Diversity)
% -------------------------------------------------------------------------
...
elseif strcmp(conf.channel_type, 'Block')
    ...
    % Apply to all symbols (including the training one for consistency)
...
% -------------------------------------------------------------------------
% Case: Block Viterbi (Phase Tracking)
% -------------------------------------------------------------------------
elseif strcmp(conf.channel_type, 'Block_Viterbi')
    ...
    for j = 1 : nb_tot_symbs
        if j == 1
            ...
            % Update H1 Phase
            % 1. Estimate raw phase from data (4th power)
            ph_raw1 = (1/4) * angle(-(rx_mat1(:,j).^4));
            % 2. Find closest to previous (Viterbi-ish)
            ...
            % Update H2 Phase (Identical logic)
